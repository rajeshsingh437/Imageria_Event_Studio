"""
IMAGERIA ANCHOR-FIRST COHORT ROUTER (ADAPTIVE SHADOW BOOST & FAN-OUT)
- Preserves the locked Phase 1 enrollment architecture.
- Adaptive Pre-Inference Exposure Normalizer: Lifts dark frames in memory
  via LAB gamma + CLAHE so RetinaFace and ArcFace reliably match shadowy faces.
- Copies the uncompressed, original raw photos to the destination folders.
- Output: [Master_Folder]\Stage2_Universal_Cohorts\
"""
import os
import sys
import shutil
...
def normalize_exposure_buffer(im_bgr: np.ndarray) -> np.ndarray:
    """
    Checks frame luminance. If underexposed or shadowed,
    applies adaptive gamma + CLAHE to the inference buffer.
    """
    # Convert to LAB color space to inspect perceptual lightness (L-channel)
    lab = cv2.cvtColor(im_bgr, cv2.COLOR_BGR2LAB)
    l_channel, a_channel, b_channel = cv2.split(lab)
    mean_l = np.mean(l_channel)

    # Threshold for underexposed / shadowy stage frames
    if mean_l < 95.0:
        # Dynamic gamma lift calibrated to darkness severity
        gamma = max(1.2, min(2.2, 100.0 / (mean_l + 1e-6)))
        inv_gamma = 1.0 / gamma
        table = np.array([((i / 255.0) ** inv_gamma) * 255 for i in np.arange(0, 256)]).astype("uint8")
        l_boosted = cv2.LUT(l_channel, table)

        # CLAHE to sharpen facial edges in deep shadows
...
def run_calibrated_pipeline(master_dir: str):
    ...
    print("=" * 75)
    print(" LOCKED ENROLLMENT & ADAPTIVE SHADOW-BOOST FAN-OUT ROUTER")
    ...
    for idx, fpath in enumerate(all_files):
        ...
        # Normalize exposure for detection
    ...
    # Cluster solo frames into unique individuals (locked logic)
    entities = []
    ...
    for ent in entities:
        first_file = ent["items"][0]["fname"]
        num_part = ''.join([c for c in first_file if c.isdigit()])
        file_num = int(num_part) if num_part else ent["first_idx"]
    ...
    # PHASE 2: Route Handshakes, Groups, and Dais Assemblies with Shadow Boost
    ...
    for idx, fpath in enumerate(multi_person_files):
        ...
        # Normalize exposure on buffer for face detection in dark shots
        im_det_norm = normalize_exposure_buffer(im_det)
        faces = app.get(im_det_norm)
        ...
        # Stage Handshake: 1 Student receiving degree from Dignitary
        ...
        # Multi-Student Group Photo: 2+ Students recognized
        ...
        # Dais & General Ceremony Assembly
