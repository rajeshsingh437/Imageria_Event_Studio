"""
IMAGERIA EVENT STUDIO — STEP 2: PRECISION FACE CLUSTERING & EVENT SORTING (LOCKED PRODUCTION)
File: step2_cohort_sorting_final_gemini.py
...
- Event Profile: "ANNUAL_DAY" (supports "CONVOCATION", "SPORTS_DAY")
- Match Calibration: Threshold=0.58 | Ambiguity Gap=0.08 (Zero lookalike misfiling)
- Companion Pairing: Keeps strictly 1 unedited master + 1 dynamically enhanced master per shot.
- Default runs on the FULL event batch unless --limit N is explicitly passed.
"""
...
DEFAULT_OUTPUT_DIR = r"E:\HTS\Step2_Sorted_Event_Cohorts"
EVENT_PROFILE = "ANNUAL_DAY"
...
def run_step2_cohort_sorting(input_dir: str, output_dir: str = DEFAULT_OUTPUT_DIR, limit: int | None = None):
    ...
    print("=" * 80)
    print(f" IMAGERIA EVENT STUDIO — STEP 2: PRECISION SORTING [{EVENT_PROFILE}] (LOCKED)")
    ...
    print(f" Execution Scope     : {'First ' + str(limit) + ' images' if limit else 'Full Event Batch (All Images)'}")
    print("=" * 80)
    ...
    if limit and limit > 0:
        ...
        print(f"[*] Processing limited batch of {len(scan_files)} images.\n")
    else:
        scan_files = all_files
        print(f"[*] Processing full production batch of {len(scan_files)} images.\n")
    ...
    # CLUSTERING: Anchor-Locked Enrollment
    ...
    # PHASE 2: Instant Memory Routing
